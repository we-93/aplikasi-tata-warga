...
import { deleteTemplate, duplicateGlobalTemplate } from "@/app/actions/letter";
...
export function LetterList({ templates }: { templates: TemplateData[] }) {
  ...
  const handleDuplicate = async (id: string) => {
    const res = await duplicateGlobalTemplate(id);
    if (res.success) {
                  ...
                  <tr key={t.id} className="bg-white dark:bg-transparent hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        <Link href={`/admin/letters/${t.id}/edit`}>
                          <button 
