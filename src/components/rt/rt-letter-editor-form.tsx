...
export function RtLetterEditorForm({ initialData }: { initialData?: TemplateData }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = isEdit 
      ? await updateRtTemplate(initialData.id!, formData)
      : await createRtTemplate(formData);

    if (res.success) {
      toast.success(isEdit ? "Template berhasil diperbarui!" : "Template berhasil disimpan!");
      router.push("/dashboard/rt/surat/template");
    } else {
